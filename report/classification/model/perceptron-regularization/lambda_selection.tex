\subsubsection{Chọn tham số bằng Stratified K-Fold}
...
\[
\min_{\boldsymbol{w}} \; \frac{1}{N} \sum_{i=1}^{N} \ell(y_i, f_{\boldsymbol{w}}(x_i)) + \lambda \Omega(\boldsymbol{w})
\]

Trong đó:
\begin{itemize}
    \item $\ell(\cdot)$ là hàm mất mát
    \item $\Omega(\boldsymbol{w})$ là hàm regularization (ví dụ: $\|\boldsymbol{w}\|^2$)
