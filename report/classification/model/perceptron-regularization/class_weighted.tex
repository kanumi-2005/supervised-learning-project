...
\[
\min_{\boldsymbol{w}} \sum_{k=1}^{K} c_k \sum_{n \in \mathcal{D}_k} \ell(y_n, f_{\boldsymbol{w}}(x_n))
\]
...
Gradient của hàm mất mát theo tham số $\boldsymbol{w}$ có dạng:
\[
\nabla_{\boldsymbol{w}} E = - \sum_{n=1}^{N} \sum_{k=1}^{K} c_k \, t_{nk} \, \nabla_{\boldsymbol{w}} \log y_{nk}
\]

Trong trường hợp Logistic Regression, điều này dẫn đến:
\[
\nabla_{\boldsymbol{w}} E \propto \sum_{n=1}^{N} c_{y_n} (y_n - \hat{y}_n)\boldsymbol{x}_n
\]
