\paragraph{Các mô hình hàm cơ sở phi tuyến}

Tất cả các mô hình đều tuân theo dạng tổng quát:

\begin{equation}
f(\boldsymbol{x}, \boldsymbol{w}) = \boldsymbol{w}^\intercal \Phi(\boldsymbol{x})
\end{equation}

trong đó $\Phi(\boldsymbol{x})$ được xây dựng từ các hệ cơ sở khác nhau.

\paragraph{Mô hình đa thức (Polynomial Basis):}
Mô hình đa thức mở rộng vector đầu vào thành các lũy thừa phi tuyến:

\begin{equation}
\Phi(\boldsymbol{x}) = [1, x, x^2, \dots, x^p]
\end{equation}
...
\begin{equation}
\phi_j(\boldsymbol{x}) =
\exp\left(-\frac{\|\boldsymbol{x} - \boldsymbol{c}_j\|^2}{2\sigma^2}\right)
\end{equation}

Trong đó $\boldsymbol{c}_j$ là các tâm và $\sigma$ điều khiển độ lan tỏa.
